...
int main(int argc, char* argv[]) {data_manager(argc, argv);}
...
void data_manager(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (list_words[i] == NULL) {
            for (int y = i-1; y >= 0; y++) free(list_words[y]);
            free(list_words);
            exit(0);
        }
    }
    ...
    if (flags.status == 0) file_manager(list_files, list_words, flags, fls);
    for (int i = 0; i < 100; i++) {
        free(list_words[i]);
    }
    free(list_words);
...
void str_manager(FILE *target, char **words, struct grep_flags flags, char* filename, int count_files) {
    char* line = NULL;
    int end_file;
    size_t size = 0;
    ...
    while ((end_file = getline(&line, &size, target)) != -1) {
        ...
        for (int i = 0; words[i][0] != 0; i++) {
            match += basic_word_finder(line, words[i], flags, 0);
            if (match == 0) flag = 1;
            if (flags.v == 0 && match == 0) {
                if (flags.c != 1 && flags.l != 1) {
                    ...
                    if (flags.o != 1) {
                        ...
                        basic_word_finder(line, words[i], flags, 1);
                    }
                } else {
                    num_matches += 1;
                }
...
int basic_word_finder(char* line, char *words, struct grep_flags flags, int print) {
    ...
    regex_t preg;
    regmatch_t mptr[1] = {0};
    int check = regcomp(&preg, words, (flags.i == 1) ? REG_ICASE | REG_EXTENDED : REG_EXTENDED);
    if (check) exit(0);
    ...
    if (print == 1) {
        while (result == 0) {
            ...
            printf("%.*s\n", (int)(mptr[0].rm_eo - mptr[0].rm_so), str + mptr[0].rm_so);
            str += mptr[0].rm_eo;
            result = regexec(&preg, str, 1, mptr, 0);
        }
    }
